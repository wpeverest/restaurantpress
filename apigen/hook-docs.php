<?php
/**
 * Generate documentation for hooks in RP
 */
class RP_HookFinder {
	...
	private static function get_hook_link( $hook, $details = array() ) {
		if ( ! empty( $details['class'] ) ) {
			$link = 'http://docs.wpeverest.com/rp-apidocs/source-class-' . $details['class'] . '.html#' . $details['line'];
		} elseif ( ! empty( $details['function'] ) ) {
			$link = 'http://docs.wpeverest.com/rp-apidocs/source-function-' . $details['function'] . '.html#' . $details['line'];
		} else {
			$link = 'https://github.com/wpeverest/restaurantpress/search?utf8=%E2%9C%93&q=' . $hook;
		}
	...
	public static function process_hooks() {
		...
		$template_files[]	= '../includes/rp-template-functions.php';
		$template_files[]	= '../includes/rp-template-hooks.php';
		...
		foreach ( self::$files_to_scan as $heading => $files ) {
			...
			foreach ( self::$custom_hooks_found as $hook => $details ) {
				if ( ! strstr( $hook, 'restaurantpress' ) && ! strstr( $hook, 'food_menu' ) && ! strstr( $hook, 'rp_' ) ) {
		...
		$html   = file_get_contents( '../rp-apidocs/tree.html' );
		...
		file_put_contents( '../rp-apidocs/hook-docs.html', $header . ob_get_clean() . end( $footer ) );
		echo "Hook docs generated :)\n";
	}
}

RP_HookFinder::process_hooks();
