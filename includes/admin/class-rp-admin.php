	...
	public function admin_footer_text( $footer_text ) {
		...
		if ( isset( $current_screen->id ) && apply_filters( 'restaurantpress_display_admin_footer_text', in_array( $current_screen->id, $rp_pages ) ) ) {
			...
			if ( ! get_option( 'restaurantpress_admin_footer_text_rated' ) ) {
				$footer_text = sprintf( __( 'If you like <strong>RestaurantPress</strong> please leave us a %s&#9733;&#9733;&#9733;&#9733;&#9733;%s rating. A huge thank you from ThemeGrill in advance!', 'restaurantpress' ), '<a href="https://wordpress.org/support/view/plugin-reviews/restaurantpress?filter=5#postform" target="_blank" class="rp-rating-link" data-rated="' . esc_attr__( 'Thanks :)', 'restaurantpress' ) . '">', '</a>' );
				rp_enqueue_js( "
